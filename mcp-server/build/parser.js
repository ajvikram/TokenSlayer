...
loadTokenizer();
export function tokenize(text, targetModel) {
    if (!text)
        return 0;
    if (_encodeFn && targetModel && text.length <= 500_000) {
        try {
            return _encodeFn(text).length;
        }
        catch { /* fall through */ }
    }
...
function processHtml(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.includes('<')) {
            ...
            const compacted = raw.replace(/<([A-Za-z][\w-]*)((?:\s[^>]*)?)>([^<>]{12,})<\/\1>/g, (_match, tag, attrs, text) => {
                if (PRESERVE_TEXT_TAGS.test(tag))
...
function processCss(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('//'))
            continue; // SCSS/Less single-line comment
...
function processVueSvelte(content, language) {
    const skeleton = [];
    ...
    if (templateMatch) {
        for (const block of templateMatch) {
            ...
            if (inner)
                skeleton.push(processHtml(inner));
    ...
    if (scriptMatch) {
        for (const block of scriptMatch) {
            ...
            if (inner)
                skeleton.push(processCLike(inner, 'typescript'));
    ...
    if (styleMatch) {
        for (const block of styleMatch) {
            ...
            if (inner)
                skeleton.push(processCss(inner));
            skeleton.push('</style>');
        }
    }
    return skeleton.join('\n');
 ...
 */
export function extractSymbol(skeleton, symbolName) {
 ...
 */
export function scoreRelevance(skeleton, filePath, query) {
 ...
 */
export function pruneToFit(skeleton, maxTokens, targetModel) {
    const estimateTokens = (s) => tokenize(s, targetModel);
    if (estimateTokens(skeleton) <= maxTokens)
        return skeleton;
    ...
    let pruned = skeleton
        ...
        .replace(/'''[\s\S]*?'''/g, '')
        .replace(/^\s*\/\/\/.*$/gm, '');
    if (estimateTokens(pruned) <= maxTokens)
        return pruned.trim();
    ...
    if (lastNewline > targetChars * 0.8)
        pruned = pruned.substring(0, lastNewline);
 ...
 */
export function optimizeLayout(skeleton, targetModel) {
    if (!targetModel)
        return skeleton;
    let result = skeleton;
    ...
    result = result.replace(/^(\t+)/gm, (_m, tabs) => '  '.repeat(tabs.length));
    result = result.replace(/^((?:    )+)/gm, (_m, spaces) => '  '.repeat(spaces.length / 4));
    // Pass 4: collapse single-line-able multi-line signatures
    result = result.replace(/^(\s*(?:export |public |private |protected |static |async |override )*(?:function|func|fn|fun|def|method)\s+\w+)\(\s*\n((?:\s+\w[^,\n]*,?\s*\n){1,5})\s*\)/gm, (_match, prefix, paramBlock) => {
 ...
 */
export function tagPrunedNodes(skeleton, filePath, originalContent) {
    const originalLines = originalContent.split('\n');
    let nodeCounter = 0;
    return skeleton.replace(/^(.+?)\s*\{\s*\/\* \.\.\. \*\/\s*\}$/gm, (match, signature) => {
 ...
 */
export function extractImportSpecifiers(content, language) {
    const specifiers = [];
    if (language === 'typescript') {
        ...
        for (const m of content.matchAll(/^\s*from\s+(\S+)\s+import/gm))
            specifiers.push(m[1]);
        ...
        for (const m of content.matchAll(/^\s*use\s+(crate::\S+?);/gm))
            specifiers.push(m[1]);
        for (const m of content.matchAll(/^\s*use\s+(super::\S+?);/gm))
            specifiers.push(m[1]);
        for (const m of content.matchAll(/^\s*mod\s+(\w+)\s*;/gm))
            specifiers.push(`mod::${m[1]}`);
    }
    else if (language === 'php') {
        for (const m of content.matchAll(/\brequire(?:_once)?\s+['"]([^'"]+)['"]/g))
            specifiers.push(m[1]);
        for (const m of content.matchAll(/\binclude(?:_once)?\s+['"]([^'"]+)['"]/g))
            specifiers.push(m[1]);
    }
 ...
 */
export function resolveImport(specifier, fromFile, language) {
    ...
    if (language === 'php' || language === 'ruby') {
        if (specifier.startsWith('./') || specifier.startsWith('../') || specifier.startsWith('/')) {
            ...
            if (fs.existsSync(candidate + lang))
 ...
 */
export function buildDependencyChain(seedFile, maxDepth = 2) {
    const MAX_FILES = 20;
    const visited = new Set();
    const results = [];
    const queue = [{ filePath: path.resolve(seedFile), depth: 0 }];
 ...
 */
export function tagAllNodes(skeleton, filePath, originalContent) {
    const originalLines = originalContent.split('\n');
...
function findOriginalLine(skeletonTrimmed, originalLines) {
 ...
 */
export function applyPatches(patches, dryRun = true) {
...
function generateDiff(filePath, original, patched) {
    const origLines = original.split('\n');
    const patchLines = patched.split('\n');
    const diffLines = [`--- a/${path.basename(filePath)}`, `+++ b/${path.basename(filePath)}`];
    let i = 0, j = 0;
    while (i < origLines.length || j < patchLines.length) {
        ...
        for (let k = hunkStart; k < origEnd; k++)
            diffLines.push(`-${origLines[k]}`);
        for (let k = hunkStartJ; k < patchEnd; k++)
            diffLines.push(`+${patchLines[k]}`);
        i = origEnd;
        j = patchEnd;
    }
