...
import type { Patch } from "./parser.js";
import {
  recordStats,
  readStats,
  aggregate,
...
class TokenSlayerServer {
  ...
  private setupToolHandlers() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      const budgetFor = (tool: keyof typeof DEFAULT_BUDGETS) =>
        (args.maxTokens as number | undefined) ?? DEFAULT_BUDGETS[tool];
      ...
      if (request.params.name === "analyze_files") {
        ...
        const expandable = args.expandable as boolean | undefined;
        const targetModel = args.targetModel as string | undefined;
        ...
        if (expandable) {
          results = results.map(r => {
            ...
            try {
              ...
              r.skeleton = tagAllNodes(
                tagPrunedNodes(r.skeleton, r.filePath, content),
                r.filePath, content
              );
            } catch { /* ignore */ }
            return r;
        ...
        if (query) {
          results.sort((a, b) =>
            scoreRelevance(b.skeleton, b.filePath, query) -
      ...
      if (request.params.name === "analyze_dependency_chain") {
        const seedFile = args.seedFile as string;
        if (!seedFile) throw new Error("seedFile is required");
      ...
      if (request.params.name === "apply_patch") {
        const patches = args.patches as Patch[];
  ...
  private findSupportedFiles(dir: string, files: string[] = []): string[] {
    try {
  ...
  private formatResults(results: any[], tool: string, format: string = 'text', maxTokens?: number, targetModel?: string) {
    let totalOriginal = 0;
    let totalCompacted = 0;
    const successful: { filePath: string; language: string; originalTokens: number; compactedTokens: number }[] = [];
